/* ============================================================
   PR Explorer · app-claude-v25.js · Midnight Teal Pro
   V2.5: stabile Sheets, POI-Overlays, Hybridkarte, Exporte
   ============================================================ */
'use strict';
...
function drawPois(){
  ...
  poiGeojson.features.forEach(f=>{
    ...
    L.marker([c[1],c[0]],{icon:ico,keyboard:false}).bindPopup(`<b>${def.icon} ${p.name||def.label}</b><br>${def.label}<br><small>OSM · ${p.id}</small>`).addTo(lgPois);
  });
}
...
function fitVisible(){ const b=allBounds(); map.flyToBounds(b,{paddingTopLeft:[28,110],paddingBottomRight:[28,140],maxZoom:14,duration:.9}); }
