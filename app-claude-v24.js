/* ============================================================
   PR Explorer · app-claude-v24.js · Midnight Teal Pro
   V2.4: OSM-Hiking-Rohdaten als editierbare Vektorlinien
         mit Kontur, Labels und deutscher Linieninfo
   ============================================================ */
'use strict';
...
let cfg = Object.assign({
  ...
  hikingVectorLabels:true, hikingVectorWeight:3, hikingVectorOpacity:.95,
  hikingVectorOutline:'auto', hikingVectorOutlineWeight:3, hikingVectorColor:'#bf5af2',
  showTestToggle:true,
...
const OVERLAY_LABELS = {
  ...
  regions:'Concelhos-Grenzen', hiking:'Waymarked Trails Raster', hikingVector:'OSM Hiking Vektor', seamarks:'OpenSeaMap Seezeichen'
};
const OVERLAY_ICONS = { tracks:'🗺️', drive:'🚗', heat:'🔥', markers:'📍', regions:'🌐', hiking:'🥾', hikingVector:'〰️', seamarks:'⚓' };
const APP_LAYER_KEYS = ['tracks','drive','heat','markers','regions','hiking','hikingVector','seamarks'];
...
function setLayer(k,on){ cfg.layers[k]=!!on; if(k==='regions')toggleRegions(); else if(k==='hikingVector')renderHikingVectorLayers(); else if(OVERLAY_TILES[k])syncOverlayTiles(); else renderLayers(); saveCfg(); renderPanel(); }
...
/* OSM HIKING VECTOR LAYER · V2.4 */
const HIKING_VECTOR_BBOX = { south:32.55, west:-17.35, north:32.95, east:-16.55 };
const HIKING_VECTOR_CACHE_KEY = 'prHikingVectorGeojsonV24';
const HIKING_VECTOR_META_KEY = 'prHikingVectorMetaV24';
...
function hikingVectorQuery(){
  const b=HIKING_VECTOR_BBOX;
  return `[out:json][timeout:120];\n(\n  relation["type"="route"]["route"~"^(hiking|foot)$"](${b.south},${b.west},${b.north},${b.east});\n);\nout tags geom;`;
}
function loadCachedHikingVector(){
  if(hikingVectorGeojson)return true;
  try{ const raw=localStorage.getItem(HIKING_VECTOR_CACHE_KEY); if(!raw)return false; hikingVectorGeojson=JSON.parse(raw); return !!hikingVectorGeojson?.features; }catch(e){ return false; }
}
...
function routeColorByProps(p){
  ...
  if(net==='iwn')return '#ff3b30'; if(net==='nwn')return '#007aff'; if(net==='rwn')return '#bf5af2'; if(net==='lwn')return '#34c759';
  const ref=(p.ref||p.name||'').toUpperCase();
  if(ref.startsWith('PR'))return '#ff9500'; if(ref.startsWith('CR'))return '#8e8e93';
  return cfg.hikingVectorColor||'#bf5af2';
}
function overpassToGeoJSON(osm){
  const features=[];
  (osm.elements||[]).filter(e=>e.type==='relation').forEach(rel=>{
    const tags=rel.tags||{};
    const lines=[];
    (rel.members||[]).forEach(m=>{
      if(m.type==='way'&&Array.isArray(m.geometry)&&m.geometry.length>1){
        lines.push(m.geometry.map(p=>[+p.lon,+p.lat]).filter(c=>isFinite(c[0])&&isFinite(c[1])));
      }
    });
...
async function fetchHikingVectorData(force=false){
  ...
  hikingVectorLoading=false; hikingVectorError=lastErr?String(lastErr.message||lastErr):'Overpass nicht erreichbar'; renderHikingVectorLayers(); renderPanel(); if(qs('#settingsContent'))renderSettings(); toast('OSM-Hiking-Rohdaten nicht geladen');
}
...
function renderSettings(){
  ...
  qs('#settingsContent').innerHTML=`
    ...
    <div class="sg"><div class="sg-title">OSM Hiking Vektorlinien</div><div class="sg-box">
      ...
      <div class="sg-row" onclick="openColorSheet('hikingVector','OSM Hiking Fallback-Farbe')"><div class="sg-icon" style="background:rgba(191,90,242,.12)">🎨</div><span class="sg-label">Fallback-Farbe</span><span class="sg-val"><div class="sg-cdot" style="background:${cfg.hikingVectorColor}"></div><svg class="sg-chev" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg></span></div>
