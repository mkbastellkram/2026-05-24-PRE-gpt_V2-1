/* ============================================================
   ...
   V2.4.4: stabiler Funktionstest, Linien-Casing, Kalenderfelder
   ============================================================ */
'use strict';
...
let cfg = Object.assign({
  ...
  layers:{ tracks:true, drive:true, heat:false, markers:true, regions:false, hiking:false, hikingVector:false, seamarks:false },
  hikingMode:'off',
...
const OVERLAY_LABELS = {
  ...
  regions:'Concelhos-Grenzen', hiking:'Hiking Referenzkarte', hikingVector:'Editierbare Hiking-Linien', seamarks:'OpenSeaMap Seezeichen'
};
const OVERLAY_ICONS = { tracks:'🗺️', drive:'🚗', heat:'🔥', markers:'📍', regions:'🌐', hiking:'🥾', hikingVector:'〰️', seamarks:'⚓' };
const APP_LAYER_KEYS = ['tracks','drive','heat','markers','regions','seamarks'];
...
const lgTrack=L.layerGroup().addTo(map), lgDrive=L.layerGroup().addTo(map), lgHeat=L.layerGroup().addTo(map), lgHikingCasing=L.layerGroup().addTo(map), lgHikingCore=L.layerGroup().addTo(map), lgHikingLabels=L.layerGroup().addTo(map), lgMarkers=L.layerGroup().addTo(map), lgRegions=L.layerGroup().addTo(map);
...
function setBase(b){ if(!TILES[b]) b='topo'; cfg.base=b;saveCfg();if(activeBase)map.removeLayer(activeBase);activeBase=TILES[b].addTo(map);syncOverlayTiles();renderHikingVectorLayers();renderPanel(); }
function setLayer(k,on){ cfg.layers[k]=!!on; if(k==='regions')toggleRegions(); else if(k==='hiking'||k==='hikingVector'){ cfg.hikingMode = cfg.layers.hikingVector ? 'vector' : (cfg.layers.hiking ? 'raster' : 'off'); syncOverlayTiles(); renderHikingVectorLayers(); } else if(OVERLAY_TILES[k])syncOverlayTiles(); else renderLayers(); saveCfg(); renderPanel(); }
...
function drawCasedPolyline(group,points,opts){ const casingWeight=+(opts.casingWeight||0); const segs=opts.splitKm?splitByJump(points,opts.splitKm):[points]; segs.forEach(seg=>{ if(!seg||seg.length<2)return; if(casingWeight>0){ L.polyline(seg,{color:opts.casingColor||'#fff',weight:(+opts.weight||3)+casingWeight,opacity:opts.casingOpacity??0.95,lineCap:'round',lineJoin:'round',smoothFactor:opts.smoothFactor||1.1}).addTo(group); } const core={color:opts.color,weight:opts.weight,opacity:opts.opacity,lineCap:'round',lineJoin:'round',smoothFactor:opts.smoothFactor||1.1}; const da=dashArr(opts.dash||'solid',opts.weight||3); if(da)core.dashArray=da; L.polyline(seg,core).addTo(group); }); }
...
function renderLayers(){
  ...
  list.forEach(r=>{
    if(cfg.layers.tracks&&r.track?.length){drawCasedPolyline(lgTrack,r.track.map(p=>[p[0],p[1]]),{color:cfg.gpxColor,weight:gw,opacity:.88,dash:cfg.gpxDash||'solid',casingColor:cfg.gpxOutlineColor||'#fff',casingWeight:+(cfg.gpxOutlineWeight||1),smoothFactor:1.1});}
    if(cfg.layers.drive&&r.driveRoute?.length){drawCasedPolyline(lgDrive,r.driveRoute,{color:cfg.kmlColor,weight:kw,opacity:.82,dash:cfg.kmlDash||'solid',casingColor:cfg.kmlOutlineColor||'#fff',casingWeight:+(cfg.kmlOutlineWeight||1),smoothFactor:1.3,splitKm:cfg.splitKmlJumps?+(cfg.kmlJumpKm||1.8):0});}
...
function syncSliders(){ const h=_cpColor.replace('#','');const r=parseInt(h.slice(0,2),16)||0,g=parseInt(h.slice(2,4),16)||0,b=parseInt(h.slice(4,6),16)||0;qs('#slR').value=r;qs('#slRv').textContent=r;qs('#slG').value=g;qs('#slGv').textContent=g;qs('#slB').value=b;qs('#slBv').textContent=b;qs('#hexInput').value=h.toUpperCase();paintColorSliders(); }
...
function addSwipeClose(el,closeFn,directions,handleOnly=false){
  ...
  el.addEventListener('touchstart',e=>{
    if(el.id==='panel' && S.tab==='test') return;
    ...
    if(directions.includes('down')&&!directions.includes('left')){
      ...
      fromHandle=((sy - rect.top) < 72) || (!isControl && (!body || body.scrollTop<=0));
    } else {
      fromHandle=true;
    }
  },{passive:true});
...
function initAllSwipe(){
  addSwipeClose(qs('#detailPanel'),closeDetail,['left','right','down']);
  addSwipeClose(qs('#panel'),()=>{if(S.tab!=='map')setTab('map');},['down']);
  addSwipeClose(qs('#filterSheet'),closeFilterSheet,['down']);
  addSwipeClose(qs('#settingsPanel'),closeSettings,['down']);
}
...
function renderTestTab(){
  ...
  let h=`<div class="test-wrap"><div class="test-header"><div style="display:flex;align-items:center;justify-content:space-between;gap:8px"><div><h2>Funktionstest ${APP_VERSION}</h2><p>${done} von ${total} geprüft · ${pct}%</p></div><button class="ts-export-sm" onclick="tcExport()" title="Ergebnisse kopieren">📋</button></div><div class="test-progress"><div class="test-progress-fill" style="width:${pct}%"></div></div></div>`;
  let num=0;
